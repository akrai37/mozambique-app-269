...
const normalizeFileName = (text) => {
    return text
        ...
        .normalize("NFD")                 // Canonical decomposition (separates base characters and diacritics)
        .replace(/[\u0300-\u036f]/g, "") // Remove diacritics
        .toLowerCase()                  // Convert to lowercase
        .replace(/\s+/g, "_")           // Replace spaces with underscores
        .replace(/[^a-z0-9_]+/g, "_")   // Collapse non-alphanumeric characters to underscores
...
const addData = async (collectionName, documentName, rootCollectionName, categoryName, subcollectionName, documentId, items) => {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let normalizedId = normalizeFileName(item[documentId]);
            const itemRef = doc(categoryRef, subcollectionName, normalizedId);
 ...
 * @param {*} items 
 */
const addLearnConvoData = async (collectionName, documentName, rootCollectionName, categoryName, subcollectionName, documentId, items) => {
    try {
        const categoryRef = doc(db, collectionName, documentName, rootCollectionName, categoryName);
        await setDoc(categoryRef, {
            name: categoryName,
        ...
        for (let i = 0; i < items.length; i++) {
            const item = stripBase64(items[i]);
            let normalizedId = normalizeFileName(item[documentId]);
            const itemRef = doc(categoryRef, subcollectionName, `${normalizedId}_${i}`);

            for (const response of item.responses) {
                response.slug = `${normalizedId}_${i}_${response.emotion}`;
            }

            await setDoc(itemRef, {
                ...item,
                slug: `${normalizedId}_${i}`,
                order: i * 1000000
            });
        }
    } catch (err) {
 ...
 * @param {*} items 
 */
const addPracticeQuizData = async (collectionName, documentName, rootCollectionName, categoryName, subcollectionName, documentId, items) => {
    try {
        const categoryRef = doc(db, collectionName, documentName, rootCollectionName, categoryName);
        await setDoc(categoryRef, {
            name: categoryName,
...
const addPracticeConvoData = async (collectionName, documentName, rootCollectionName, categoryName, subcollectionName, documentId1, documentId2, items) => {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let subDocId = item[documentId1] || item[documentId2];

            subDocId = subDocId.split('/').pop().split('.')[0];

            const itemRef = doc(categoryRef, subcollectionName, subDocId);
            await setDoc(itemRef, {
                ...item,
                slug: subDocId,
                order: i * 1000000
...
const addHomeData = async (collectionName, documentName, rootCollectionName, items) => {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            item.word = normalizeFileName(item.word);
...
const addCardsToFirestore = async () => {
    ...
    for (const category in learnConvo) {
        await addLearnConvoData(collectionName, learnConvoDocumentName, "categories", category, "conversations", "questionText", learnConvo[category]);
    ...
    for (const category in practiceQuiz) {
        await addPracticeQuizData(collectionName, practiceQuizDocumentName, "categories", category, "quizzes", "questionText", practiceQuiz[category]);
