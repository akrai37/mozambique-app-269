...
const pracconvo = '../../assets/json/prac_convo.json';
...
const questions = JSON.parse(fs.readFileSync(questionJson, 'utf-8'));
const convo = JSON.parse(fs.readFileSync(pracconvo, 'utf-8'));

const collectionName = 'cards';
const catDocumentName = 'pracConvo';
...
const addCardsToFirestore = async () => {
    for (const category in convo) {
        const items = convo[category];
