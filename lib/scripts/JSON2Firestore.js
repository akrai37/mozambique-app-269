...
const addHomeData = async (collectionName, documentName, rootCollectionName, items) => {
    try {
        ...
        await setDoc(docRef, {
            [rootCollectionName]: items.map((item, index) => ({
                ...stripBase64(item),
                order: index
            }))
        }, { merge: true });
...
const addCardsToFirestore = async () => {
    ...
    for (const category in practiceQuiz) {
        await addData(collectionName, practiceQuizDocumentName, "categories", category, "quizzes", "questionText", practiceQuiz[category]);
    ...
    for (const category in practiceConvo) {
        await addPracticeConvoData(collectionName, practiceConvoDocumentName, "categories", category, "conversations", "imagePath", "audioPath", practiceConvo[category]);
