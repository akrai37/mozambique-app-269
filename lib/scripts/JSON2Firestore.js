...
const addData = async (collectionName, documentName, data) => {
    try {
        // Remove imageBase64 and audioBase64 fields from data if they exist
...
/**
 * Add data to Firestore with base64 handling
 * This function will create a document in the specified collection with the given data
 * 
 * @param {string} collectionName 
 * @param {string} documentName 
 * @param {*} data 
 */
const addDataWithBase64 = async (collectionName, documentName, data) => {
    try {
        const collRef = collection(db, collectionName);
        const docRef = doc(collRef, documentName);
        await setDoc(docRef, data, { merge: true });
...
const addCardsToFirestore = async () => {
    ...
    for (const category in practiceConvo) {
        const items = practiceConvo[category];
        const data = { [category]: items };
        await addData(collectionName, practiceConvoDocumentName, data);
